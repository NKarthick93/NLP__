...
def dat(a,b):
  ...
  return ar  #returns a list of matched skills
#########################################################################################################################################################################
def resume(path):
    #text = re.sub('\n', ' ', text)
    #text = re.sub('\s+', ' ', text)
    ...
    return dataframe_result_res
    ########################################################
def Jd(path1):    
    text_jd = get_text(path1)
    ...
    return dataframe_result_jd
    ########################################################
    #dataframe_final = [dataframe_result_res,dataframe_result_jd]
@app.route('/resume', methods=['GET','POST'])
def output():
  if request.method == 'POST':
    text_res = request.files['text_res']
    text_jd = request.files['text_jd']
    text_res.save(text_res.filename)
    text_jd.save(text_jd.filename)
    dataframe_result_res1 = resume(text_res.filename)
    dataframe_result_jd1 = Jd(text_jd.filename)
    Final_data = pd.concat([dataframe_result_jd1, dataframe_result_res1], axis=1, ignore_index=False)
    ...
    #result = {'The JD and RESUME MATCHING SCORE PERCENTAGE':per}
    df = Datasets.copy()
    df = df.replace('NaN','N/A')
    df = df.replace('null','N/A')
    df = df.replace('nan','N/A')
    df['MATCHING_SCORE'] = df['MATCHING_SCORE'].fillna('N/A')
    df = df.to_json(orient='records')
    df = json.loads(df)
    data = {'The JD and RESUME MATCHING SCORE PERCENTAGE':per, 'Details':df}
    #data = {'SKILLS_FROM_JD':df['SKILLS_FROM_JD'],'SKILLS_FROM_RESUME':df['SKILL_FROM_RESUME'],'DECISION_WORDS_JD':df['DECISION_WORDS_JD'],'WORK_EXPERIENCE_JD':df['WORK_EXPERIENCE_JD'],'WORK_EXPERIENCE_RESUME':df['WORK_EXPERIENCE_RESUME'],'MATCHING_SCORE':df['MATCHING_SCORE']}
    #data ={}
    #for i in range(len(df)):
        #data = {'SKILLS_FROM_JD':df.iloc['SKILLS_FROM_JD'],'SKILLS_FROM_RESUME':df.iloc['SKILL_FROM_RESUME'],'DECISION_WORDS_JD':df.iloc['DECISION_WORDS_JD'],'WORK_EXPERIENCE_JD':df.iloc['WORK_EXPERIENCE_JD'],'WORK_EXPERIENCE_RESUME':df.iloc['WORK_EXPERIENCE_RESUME'],'MATCHING_SCORE':df.iloc['MATCHING_SCORE']}
    #print(Datasets)
    #print(data)
    return data
    ...
    #df = df.to_dict()
    #return (result,data)
    #print(Datasets.head(6))
    #print('\n')
    #print("The JD and Resume Matching score is: ",Matching_score,'%')
    #Datasets.to_csv('cv.csv')
